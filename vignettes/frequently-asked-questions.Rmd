...
We can fix these ugly labels using a **ggplot2** command and text wrapping via the **stringr** package. You might need to play around with the `str_wrap()` parameter to get the breaks looking nice. This leaves the category variable in the data alone (since `str_wrap` would otherwise convert the type to character and lose factor level ordering, etc).

```{r wrapped_labels, message=FALSE}

xlabs <- levels(hhinc19_custom$HRACE_custom) %>% stringr::str_wrap(12)
